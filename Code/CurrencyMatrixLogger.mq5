//+------------------------------------------------------------------+
//|                CurrencyMatrixLogger.mq5                          |
//|   Multi-Pair M1 OHLCV Logger für Basiswährung                    |
//|   Schreibt CSV pro Basiswährung im /Files-Verzeichnis (append)   |
...
{
    SetPairs();
    CSVFile = CSVPrefix + BasisWaehrung + "_M1.csv";
...
void LogCurrentBar()
{
    // Hole die Zeit der letzten abgeschlossenen M1-Bar
    MqlRates rates[];
...
bool WriteBar(datetime bar_time, bool isBackfill)
{
    ...
    for(int i=0; i<ArraySize(EA_Pairs); i++)
    {
        ...
        double spread = UseSpread ? SymbolInfoDouble(pair, SYMBOL_SPREAD) : 0;
        double bid    = UseBidAsk ? SymbolInfoDouble(pair, SYMBOL_BID)    : 0;
        double ask    = UseBidAsk ? SymbolInfoDouble(pair, SYMBOL_ASK)    : 0;

        FileWrite(fileHandle,
            timestr, pair, rates[0].open, rates[0].high, rates[0].low, rates[0].close, rates[0].tick_volume,
            spread, bid, ask, BasisWaehrung, isBackfill ? "BACKFILL" : "LIVE", __VERSION__, AccountInfoInteger(ACCOUNT_LOGIN)
        );
    }
...
{
    string statusFile = CSVPrefix + BasisWaehrung + "_status.txt";
...
{
    string errFile = CSVPrefix + BasisWaehrung + "_error.log";
...
void SetPairs()
{
    if(BasisWaehrung=="AUD") EA_Pairs = AUDPairs;
    else if(BasisWaehrung=="EUR") EA_Pairs = EURPairs;
    else if(BasisWaehrung=="CHF") EA_Pairs = CHFPairs;
    else if(BasisWaehrung=="GBP") EA_Pairs = GBPPairs;
    else if(BasisWaehrung=="USD") EA_Pairs = USDPairs;
    else if(BasisWaehrung=="NZD") EA_Pairs = NZDPairs;
    else if(BasisWaehrung=="CAD") EA_Pairs = CADPairs;
    else if(BasisWaehrung=="JPY") EA_Pairs = JPYPairs;
    else
    {
