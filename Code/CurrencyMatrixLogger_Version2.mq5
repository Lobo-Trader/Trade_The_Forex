...
bool WriteBar(datetime bar_time, bool isBackfill)
{
    ...
    for(int i=0; i<ArraySize(EA_Pairs); i++)
    {
        ...
        double spread = 0, bid = 0, ask = 0;
        if(UseSpread) SymbolInfoDouble(pair, SYMBOL_SPREAD, spread);
        if(UseBidAsk) SymbolInfoDouble(pair, SYMBOL_BID, bid);
        if(UseBidAsk) SymbolInfoDouble(pair, SYMBOL_ASK, ask);
